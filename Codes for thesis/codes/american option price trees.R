...
price_american_put <- function(stock_tree, q, r, dt, K) {
  n <- nrow(stock_tree) - 1
  ...
  for (i in n:1) {
    for (j in 1:i) {
      continuation_value <- ((1 - q) * option_tree[i + 1, j] +
                               q * option_tree[i + 1, j + 1]) / exp(r * dt)
      
      exercise_value <- K - stock_tree[i, j]
...
crr_tree  <- price_american_put(crr_stock, q_crr, r, dt, K)
...
tian_tree  <- price_american_put(tian_stock, q_tian, r, dt, K)
...
jr_tree  <- price_american_put(jr_stock, q_jr, r, dt, K)
...
haahtela_tree  <- price_american_put(haahtela_stock, q_haahtela, r, dt, K)

#Plot four trees in one frame
layout(matrix(1:4, nrow = 2, byrow = TRUE))

plot_tree(crr_tree, "CRR American Put Price Tree")
plot_tree(tian_tree, "Tian American Put Price Tree")
plot_tree(jr_tree, "Jarrow-Rudd American Put Price Tree")
plot_tree(haahtela_tree, "Haahtela American Put Price Tree")